...
pipenv run python scripts/mutcmt2json.py /tmp/sars2-mutation-comments.csv downloads/mutation-comments/$(date +'%Y%m%d').json
rm -rf downloads/muation-comments/latest.json 2>/dev/null || true
ln -s $(date +'%Y%m%d').json downloads/mutation-comments/latest.json
